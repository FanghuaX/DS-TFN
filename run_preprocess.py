...
from preprocess import save_sparse, save_data, save_subgraphs

from preprocess.parse_csv import VehicleParser,Mimic3Parser, Mimic4Parser, EICUParser
...
from preprocess.auxiliary import generate_code_code_adjacent, generate_code_code_adjacent_and_temporal, generate_neighbors, normalize_adj, divide_middle, generate_code_levels, generate_subgraphs_for_patients
...
if __name__ == '__main__':
    ...
    data_path = 'data'
    dataset = 'vehicle'  #
    dataset_path = os.path.join(data_path, dataset) # dataset_path = /Chet-master/data/mimic3/
    raw_path = os.path.join(dataset_path, 'raw')  # 读取数据路径：/Chet-master/data/mimic3/raw
    if not os.path.exists(raw_path):
        os.makedirs(raw_path)
        print('please put the CSV files in `data/%s/raw`' % dataset)
        exit()
    parsed_path = os.path.join(dataset_path, 'parsed') # 新增parsed_path路径，/Chet-master/data/mimic3/parsed
    if from_saved:
        ...
        patient_admission, admission_codes = parser.parse(sample_num) # 返回self.patient_admission, self.admission_codes
    ...
    adj_cooccur, adj_temporal = generate_code_code_adjacent_and_temporal(pids=train_pids, patient_admission=patient_admission,
                                           admission_codes_encoded=admission_codes_encoded,
                                           code_num=code_num, threshold=conf[dataset]['threshold'])
    ...
    # 产生Global diagnosis-neighbor graph，
    print('generating train neighbors ...')
    train_neighbors = generate_neighbors(train_code_x, train_visit_lens, adj_cooccur)
    print('generating valid neighbors ...')
    valid_neighbors = generate_neighbors(valid_code_x, valid_visit_lens, adj_cooccur)
    print('generating test neighbors ...')
    test_neighbors = generate_neighbors(test_code_x, test_visit_lens,  adj_cooccur)
    ...
    save_data(train_path, train_code_x, train_visit_lens, train_codes_y, train_neighbors)
    print('\tsaving valid data')
    save_data(valid_path, valid_code_x, valid_visit_lens, valid_codes_y,valid_neighbors)
    print('\tsaving test data')
    save_data(test_path, test_code_x, test_visit_lens, test_codes_y,test_neighbors)

    adj_cooccur = normalize_adj(adj_cooccur)
    adj_temporal = normalize_adj(adj_temporal)
    save_sparse(os.path.join(standard_path, 'adj_cooccur'), adj_cooccur)
    save_sparse(os.path.join(standard_path, 'adj_temporal'), adj_temporal)
