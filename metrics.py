...
def evaluate_codes(code_adj, code_adj1, model, dataset, loss_fn, output_size, historical=None):
    model.eval() # 开始验证
    total_loss = 0.0
    labels = dataset.label()  # 取出y
    preds = []
    for step in range(len(dataset)):
        code_x, visit_lens, y, neighbors = dataset[step]
        output = model(code_adj,code_x, visit_lens)
        pred = torch.argsort(output, dim=-1, descending=True) # 概率排序
        preds.append(pred)
    ...
    prec, recall = top_k_prec_recall(labels, preds, ks=[2, 5, 8, 11, 14, 17, 20, 23,26])
    if historical is not None:
        r1, r2 = calculate_occurred(historical, labels, preds, ks=[2, 5, 8, 11, 14, 17, 20, 23,26])
        sun11 = f1_score+recall[0]+recall[1]+recall[2]+recall[3]+recall[4]+recall[5]+ recall[6]+recall[7]+recall[8]
        print('\r    Evaluation: loss: %.4f---- Sum: %.4f --- f1_score: %.4f --- top_k_recall: %.4f, %.4f, %.4f, %.4f,%.4f, %.4f, %.4f, %.4f, %.4f   --- occurred: %.4f, %.4f, %.4f, %.4f,%.4f, %.4f, %.4f, %.4f, %.4f  --- not occurred: %.4f, %.4f, %.4f, %.4f,%.4f, %.4f, %.4f, %.4f, %.4f'
              % (avg_loss, sun11, f1_score, recall[0], recall[1], recall[2], recall[3],recall[4], recall[5], recall[6], recall[7],recall[8], r1[0], r1[1], r1[2], r1[3],r1[4], r1[5], r1[6], r1[7], r1[8], r2[0], r2[1], r2[2], r2[3], r2[4], r2[5], r2[6], r2[7], r2[8]))
    else:
        print('\r    Evaluation: loss: %.4f --- f1_score: %.4f --- top_k_recall: %.4f, %.4f, %.4f, %.4f,%.4f, %.4f, %.4f, %.4f'
              % (avg_loss, f1_score, recall[0], recall[1], recall[2], recall[3],recall[4], recall[5], recall[6], recall[7]))
    return avg_loss, f1_score


def evaluate_hf(code_adj, code_adj1, model, dataset, loss_fn, output_size=1, historical=None):
    model.eval()
    total_loss = 0.0
    ...
    preds = []
    for step in range(len(dataset)):
        code_x, visit_lens, y, neighbors, cooccur_subgraphs, temporal_subgraphs = dataset[step]
        output = model(code_adj, code_adj1, code_x, visit_lens, cooccur_subgraphs, temporal_subgraphs).squeeze()
        loss = loss_fn(output, y)
        total_loss += loss.item() * output_size * len(code_x)
